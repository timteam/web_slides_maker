...
this.getNewFileName = function ( id, originalFileName ) {
	return id + '.' + originalFileName.split( '.' )
		.pop();
}
...
this.parseJsonFilesFromDirectoryPath = function ( chemin, cb ) {
	var resultList = [];
	fs.readdir( chemin, ( err, files ) => {
		...
		files.forEach( filename => {
			...
			if ( path.extname( filename ) === "." + 'json' ) {
				this.readFileIfExists( chemin + filename, ( err, data ) => {
					if ( err ) {
						console.log( 'error' );
						throw err;
					}
					resultList.push( JSON.parse( data ) );

					if ( cb && files.length === fileNb ) {
						cb( null, resultList );
					}
				} );
			}
